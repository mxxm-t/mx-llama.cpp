...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    void disable_seq(llama_seq_id seq_id, const char * reason) {
        ...
        if (all_disabled() && !pre_norm_disabled) {
            llama_set_embeddings_pre_norm(params.ctx_tgt, false, false);
            llama_set_embeddings_pre_norm(params.ctx_dft, false, true);
        ...
        , params(params.draft)
    {
        ...
        // disable. Set, it enables deferred-prefill, the kv-only replay, the draft-ring disable,
        // and the non-finite-draft fail-safe (disable MTP per-seq when a draft probability is not
        // finite, falling back to target-only decode).
        static const bool mtp_opt = std::getenv("LLAMA_ENABLE_MTP_OPT") != nullptr;
        adaptive_disable = mtp_opt;
        // Position-indexed accum assumes a single sequence per request (prompt positions of
        // different sequences would collide), so gate deferred prefill to n_seq == 1.
        deferred_prefill = mtp_opt && (n_seq == 1);
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        if (adaptive_disable && seq_id >= 0 && seq_id < (llama_seq_id) disabled.size()) {
            ...
            if (pre_norm_disabled) {
                llama_set_embeddings_pre_norm(params.ctx_tgt, true, /*masked*/ false);
                llama_set_embeddings_pre_norm(params.ctx_dft, true, /*masked*/ true);
    ...
    bool process_decode(int32_t n_tokens, const llama_token * tokens, const llama_pos * positions,
        ...
        const float * h_tgt = h_tgt_in ? h_tgt_in : llama_get_embeddings_pre_norm(ctx_tgt);
        ...
        if (rc != 0) {
            LOG_ERR("%s: llama_decode(ctx_dft) failed rc=%d (pos=%d)\n", __func__, (int) rc, (int) positions[0]);
            return false;
        }
    ...
    void finish_deferred_prefill() {
        ...
        // draft context so graph_mtp builds the cheap store-only graph. Cleared after the replay so
        // generation drafting (which needs the head logits) uses the full graph.
        const bool kv_only = ctx_dft != nullptr;
        if (kv_only) {
